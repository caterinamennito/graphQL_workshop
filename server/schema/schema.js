...
const {
  ...
  GraphQLID,
  GraphQLList
} = graphql; // 2. ici on va extraire les objets dont on a besoin du package graphql
...
const WatchType = new GraphQLObjectType({
  ...
  fields: () => ({
    id: { type: GraphQLID },
    octocatId: {
      // 10. on va définir le type et matcher via le resolver avec l'octocat correspondant
      type: UserType,
      resolve(parent, args) {
        // on va chercher l'octocat associé
        // console.log(parent);
...
const RootQuery = new GraphQLObjectType({
  // 5. represents all of the possible entry points into the GraphQL API
  ...
  fields: {
    octocat: {
      ...
      resolve(parent, args) {
        // 8. lorsqu'on fera une query (dans GraphiQL), c'est la fonction resolve qui se lance
        // #2 - request API & return item
        return axios
          .get("https://api.sheety.co/97ac06ad-6ce3-4719-ad8d-8ea8711b328b")
    ...
    octocats: {
      ...
      resolve(parent, args) {
        // 12. on retourne la liste JSON de notre API octocats dorenavant
...
module.exports = new GraphQLSchema({
  // 6. ici on va exporter notre schèma
  query: RootQuery
});
